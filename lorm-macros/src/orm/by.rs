...
pub fn generate_by(
    ...
    let stream: Vec<(TokenStream, TokenStream)> = model.fields.iter().filter(|field| field.should_generate_selector(&model.primary_key)).map(|field| {
    ...
    let primary_key_by: Box<dyn Iterator<Item = (TokenStream, TokenStream)>> = Box::new(
        match &model.primary_key {
            ...
            PrimaryKey::Manual(fields) => {
                if fields.len() > 1 {
                    let by_fn_name = &model.primary_key_selector;
                    let lifetime = quote! {'a};
